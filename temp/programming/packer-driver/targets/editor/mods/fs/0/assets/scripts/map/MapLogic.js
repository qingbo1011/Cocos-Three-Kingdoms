    ...
    execute: function () {
      ...
      _export("default", MapLogic = (_dec = ccclass('MapLogic'), _dec(_class = (_temp = class MapLogic extends Component {
        ...
        onMouseWheel(event) {
          ...
          let changeRatio = Number((scrollY / this._changeZoomRadix).toFixed(1));
          let newZoomRatio = Math.min(this._minZoomRatio, Math.max(this._maxZoomRatio, this._mapCamera.orthoHeight / this._orthoHeight + changeRatio));
        ...
        onTouchEnd(event) {
          ...
          if (this._isMove == false) {
            let touchLocation = event.touch.getUILocation();
            let touchLocation1 = this.viewPointToWorldPoint(touchLocation);
            let mapPoint = (_crd && MapUtil === void 0 ? (_reportPossibleCrUseOfMapUtil({
              error: Error()
            }), MapUtil) : MapUtil).worldPixelToMapCellPoint(touchLocation1);
            let clickCenterPoint = (_crd && MapUtil === void 0 ? (_reportPossibleCrUseOfMapUtil({
              error: Error()
        ...
        viewPointToWorldPoint(point) {
          ...
          let cameraWorldX = uit.width * uit.anchorX - view.getVisibleSize().width * cuit.anchorX + this._mapCamera.node.position.x;

          let cameraWorldY = uit.height * uit.anchorY - view.getVisibleSize().height * cuit.anchorY + this._mapCamera.node.position.y;
        ...
        worldToMapPixelPoint(point) {
          ...
          let pixelX = point.x - uit.width * uit.anchorX;
          let pixelY = point.y - uit.height * uit.anchorY;
          return new Vec2(pixelX, pixelY);
        }

        scrollToMapPoint(point) {
          let pixelPoint = (_crd && MapUtil === void 0 ? (_reportPossibleCrUseOfMapUtil({
            error: Error()
          }), MapUtil) : MapUtil).mapCellToPixelPoint(point); // console.log("scrollToMapPoint", pixelPoint.x, pixelPoint.y);

          let positionX = Math.min(this._maxMapX, Math.max(-this._maxMapX, pixelPoint.x));
          let positionY = Math.min(this._maxMapY, Math.max(-this._maxMapY, pixelPoint.y));
