  ...
  return {
    ...
    execute: function () {
      ...
      _export("GeneralData", GeneralData = class GeneralData {
        ...
        static createFromServer(serverData, generalData = null, generalCfg) {
        ...
        static getPrValue(pr = 0, group, add = 0) {
          return (pr + group + add) / 100;
        }

        static getPrStr(pr = 0, add = 0, lv = 0, grow = 0) {
      ...
      _export("default", GeneralProxy = class GeneralProxy {
        ...
        initGeneralTex(texs) {
          ...
          for (let i = 0; i < texs.length; i++) {
            let id = Number(String(texs[i].name).split("_")[1]);
        ...
        updateMyGenerals(datas) {
          for (var i = 0; i < datas.length; i++) {
            let data = GeneralData.createFromServer(datas[i], null, this._generalConfigs.get(datas[i].cfgId));
        ...
        updateGeneral(data) {
          if (data.state != 0) {
            ...
            let ids = [];
            let general = GeneralData.createFromServer(data, this._myGenerals.get(data.id), this._generalConfigs.get(data.cfgId));
        ...
        getGeneralIds(cfgId) {
          let myGenerals = this.getMyGenerals();
          let tempGenerals = [];
        ...
        getComposeGenerals(cfgId = 0, id = 0) {
          var temp = [];
          var tempArr = this.getMyGenerals().concat();
