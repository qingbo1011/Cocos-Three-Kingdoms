    ...
    execute: function () {
      ...
      _export("default", MapUtil = class MapUtil {
        ...
        static initMapConfig(map) {
          ...
          let showH = Math.min(Math.ceil(vsize.height / this._tileSize.height / 2) * 2 + 2, this._mapSize.height);
        ...
        static get9GridVaildAreaIds(id) {
          let list = [];
          let totalList = this.get9GridAreaIds(id);
        ...
        static getAreaIdByCellPoint(x, y) {
          let point = this.getAreaPointByCellPoint(x, y);
        ...
        static getVaildAreaIdsByPixelPoints(...points) {
          let list = [];

          for (let i = 0; i < points.length; i++) {
            let cellPoint = this.mapPixelToCellPoint(points[i]);
            let areaPoint = this.getAreaPointByCellPoint(cellPoint.x, cellPoint.y);
            let index = this.getIdByAreaPoint(areaPoint.x, areaPoint.y);
        ...
        static worldPixelToMapCellPoint(point) {
          ...
          let x = Math.floor(0.5 * this._mapSize.height + point.x / this._tileSize.width - point.y / this._tileSize.height);
          let y = Math.floor(1.5 * this._mapSize.width - point.x / this._tileSize.width - point.y / this._tileSize.height);
          return v2(x, y);
        } //地图坐标(格子的中心点)转世界像素坐标


        static mapCellToWorldPixelPoint(point) {
          let pixelX = this._zeroPixelPoint.x - (point.y - point.x) * this._tileSize.width * 0.5;
          let pixelY = this._zeroPixelPoint.y - (point.x + point.y) * this._tileSize.height * 0.5;
          return v2(pixelX, pixelY);
        } // 地图坐标转地图像素坐标


        static mapCellToPixelPoint(point) {
          let worldPoint = this.mapCellToWorldPixelPoint(point);
        ...
        static mapPixelToCellPoint(point) {
          ...
          let worldPoint = temp.add(this._mapOffsetPoint);
          return this.worldPixelToMapCellPoint(worldPoint);
        }

        static armyIsInView(x, y) {
          let buildProxy = (_crd && MapCommand === void 0 ? (_reportPossibleCrUseOfMapCommand({
            error: Error()
          }), MapCommand) : MapCommand).getInstance().buildProxy;
          let cityProxy = (_crd && MapCommand === void 0 ? (_reportPossibleCrUseOfMapCommand({
            error: Error()
          }), MapCommand) : MapCommand).getInstance().cityProxy;
          let myId = cityProxy.getMyPlayerId();
          let myUnionId = cityProxy.myUnionId; // let parentId = cityProxy.myParentId;
          //可视觉区域以当前为原点，半径为5
          ...
          for (let i = Math.max(0, x - 5); i <= Math.min(x + 5, this._mapSize.width); i++) {
            for (let j = Math.max(0, y - 5); j <= Math.min(y + 5, this._mapSize.height); j++) {
              let id = MapUtil.getIdByCellPoint(i, j);
