    ...
    execute: function () {
      ...
      _export("default", MapTouchLogic = (_dec = ccclass('MapTouchLogic'), _dec2 = property(Prefab), _dec3 = property(Node), _dec(_class = (_class2 = (_temp = class MapTouchLogic extends Component {
        ...
        onTouchMap(mapPoint, clickPixelPoint) {
          ...
          var cityData = this._cmd.cityProxy.getCity(cellId);
          ...
          var buildData = this._cmd.buildProxy.getBuild(cellId);
          ...
          var resData = this._cmd.proxy.getResData(cellId);
          ...
          if (resData.type > 0) {
            ...
            if (temp) {
              ...
              var _cellId = (_crd && MapUtil === void 0 ? (_reportPossibleCrUseOfMapUtil({
                error: Error()
              }), MapUtil) : MapUtil).getIdByCellPoint(temp.x, temp.y);

              var _buildData = this._cmd.buildProxy.getBuild(_cellId);

              if (_buildData) {
                this.showClickUINode(_buildData, clickPixelPoint);
              } else {
