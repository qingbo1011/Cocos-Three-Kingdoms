    ...
    execute: function () {
      ...
      _export("default", MapScene = (_dec = ccclass('MapScene'), _dec2 = property(Node), _dec(_class = (_class2 = (_temp = class MapScene extends Component {
        ...
        onLoad() {
          ...
          var tiledMap = this.mapLayer.addComponent(TiledMap);
          ...
          this.scheduleOnce(() => {
            var myCity = this._cmd.cityProxy.getMyMainCity();
        ...
        onTimer() {
          if (this._cmd.proxy.qryAreaIds && this._cmd.proxy.qryAreaIds.length > 0) {
            var qryIndex = this._cmd.proxy.qryAreaIds.shift();

            var qryData = this._cmd.proxy.getMapAreaData(qryIndex);
          ...
          if (nowTime - this._lastUpPosTime > 1000) {
            ...
            var point = (_crd && MapCommand === void 0 ? (_reportPossibleCrUseOfMapCommand({
              error: Error()
        ...
        onMapShowAreaChange(centerPoint, centerAreaId, addIds, removeIds) {
          ...
          var resLogic = this.node.getComponent(_crd && MapResLogic === void 0 ? (_reportPossibleCrUseOfMapResLogic({
            error: Error()
          }), MapResLogic) : MapResLogic);
          var buildResLogic = this.node.getComponent(_crd && MapResBuildLogic === void 0 ? (_reportPossibleCrUseOfMapResBuildLogic({
            error: Error()
          }), MapResBuildLogic) : MapResBuildLogic);
          var buildFacilityLogic = this.node.getComponent(_crd && MapFacilityBuildLogic === void 0 ? (_reportPossibleCrUseOfMapFacilityBuildLogic({
            error: Error()
          }), MapFacilityBuildLogic) : MapFacilityBuildLogic);
          var tagLogic = this.node.getComponent(_crd && MapBuildTagLogic === void 0 ? (_reportPossibleCrUseOfMapBuildTagLogic({
            error: Error()
          }), MapBuildTagLogic) : MapBuildTagLogic);
          var buildTipsLogic = this.node.getComponent(_crd && MapBuildTipsLogic === void 0 ? (_reportPossibleCrUseOfMapBuildTipsLogic({
            error: Error()
          }), MapBuildTipsLogic) : MapBuildTipsLogic);
          var cityLogic = this.node.getComponent(_crd && MapCityLogic === void 0 ? (_reportPossibleCrUseOfMapCityLogic({
            error: Error()
          }), MapCityLogic) : MapCityLogic);
          var sysCityLogic = this.node.getComponent(_crd && MapSysCityLogic === void 0 ? (_reportPossibleCrUseOfMapSysCityLogic({
            error: Error()
          ...
          for (var i = 0; i < addIds.length; i++) {
            var areaData = this._cmd.proxy.getMapAreaData(addIds[i]); // console.log("areaData", areaData);


            for (var x = areaData.startCellX; x < areaData.endCellX; x++) {
              for (var y = areaData.startCellY; y < areaData.endCellY; y++) {
